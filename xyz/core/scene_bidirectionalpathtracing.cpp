...
void Scene::EvaluateBidirectionalPathTracing(
  ...
  // 視点から光源へ向けて経路を生成する．
  BuildTheEyePath(x, y, theEyePathVertices, sample);
...
void Scene::BuildLightsPath(__inout std::vector<PathVertex> &lightsPathVertics,
  ...
  for (std::size_t s = 1;;) {
    ...
    if (Bsdf::LIGHT == pBSDF->What()) {
      newLightsPathVertex.pGeometry = nullptr;
      return;
    }
    ...
    if (++s >= XYZ_CONFIG_kMaxRandomWalkDepth) {
      lightsPathVertics[s + 1].pGeometry = nullptr;
      break;
    }
