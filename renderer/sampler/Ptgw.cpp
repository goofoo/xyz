...
namespace tgir
{
  void Ptgw::run()
  {
    std::tcerr << _TEXT("** Path Tracing with Go-with-the-Winners **") <<
std::endl;
    ...
    {
#pragma omp parallel for private(rad)
      for (int y = 0; y < H; ++y)
      {
        std::mt19937_64 & random = randoms[omp_get_thread_num()];
        for (int x = 0; x < W; ++x)
        {
          scene.EvaluteGoWithTheWinner(x, y, rad, random);
          film[y*W+x] += rad;
        }
      }
      ...
      // 定期的な保存
      {
        ...
        std::clock_t const time = static_cast<std::clock_t>(currentTime /
double(CLOCKS_PER_SEC) + 0.5);
        ...
        if (is_finished || (time >= saveTime))
        {
          ...
          ::_stprintf_s(filename, MAX_PATH,
            ...
            tgir::GetOutdirPath().c_str(),
            time/(60*60), (time/60) % 60, time % 60,
            spp, scene.n_FindIntersection);
