...
namespace tgir {
void SimplifiedMLT::run() {
  std::tcerr << _TEXT("** Simplified Metropolis Light Transport **")
             << std::endl;
  ...
  struct ThreadData {
    ThreadData() : acceptanceRatio(0) {}

    ThreadData(ThreadData const &) : acceptanceRatio(0) {}
...
#pragma omp parallel for
      for (int i = 0; i < CONFIG_SEEDS; ++i) {
        ...
        seeds[i].q = seeds[i].paths.PDFSimplifiedMLT();
        cdf[i + 1] = seeds[i].q;
      }
...
#pragma omp parallel for
    for (int c = 0; c < TGIR_CONFIG_kSystemCount; ++c) {
      ...
      for (std::size_t n = 0; n < N; ++n) {
        ...
        int c;
        if (data.std::uniform_real_distribution<tgir::Real>()(random) <
            TGIR_CONFIG_kLargeStepProb) {
          c = 1;
        ...
        newSample.q = newSample.paths.PDFSimplifiedMLT();
        ...
        m.Deposite(
            oldSample.values,
            (1 - a) * hi::rcp(oldSample.q * b + TGIR_CONFIG_kLargeStepProb));
        m.Deposite(
            newSample.values,
            (c + a) * hi::rcp(newSample.q * b + TGIR_CONFIG_kLargeStepProb));
