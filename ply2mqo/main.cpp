...
static int tristrips_cb(p_ply_argument argument) {
  ...
  if (value_index < 0) {
    ...
    tristrips[value_index] = (int)ply_get_argument_value(argument);
      ...
      fprintf(fp, "  face %ld {\n", nfaces);
      {
        ...
        for (std::size_t i = 0, size = tristrips.size(); i < size; ++i) {
          if (tristrips[i] < 0) {
            ...
            if (flip) {
              fprintf(fp, "    3 V(%d %d %d)\n", tristrips[i - 2],
                      tristrips[i - 1], tristrips[i]);
            } else {
              fprintf(fp, "    3 V(%d %d %d)\n", tristrips[i - 1],
                      tristrips[i - 2], tristrips[i]);
            }
            flip = !flip;
          }
        }
...
static int convert(char filename[], FILE *fp) {
  ...
  fputs(
      ...
      fp);
  fputs(
      "  pos 0.0000 0.0000 400.0000"
      "\n",
      fp);
  fputs(
      "  lookat 0.0000 000.0000 0.0000"
      "\n",
      fp);
  fputs(
      "  head 0.0000"
